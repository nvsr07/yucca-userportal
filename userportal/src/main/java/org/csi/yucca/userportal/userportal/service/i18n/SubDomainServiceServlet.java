...
public class SubDomainServiceServlet extends I18nServlet {
	...
	protected JSONObject loadMessages(Locale currentLocale) throws MalformedURLException {
		...
		try {
			...
			is = new URL(tagsDomainsURL + "/userportal/api/proxy/services/misc/streamsubdomains/").openStream();
	...
	protected String formatMessages(JSONObject messages, Locale locale) {
		...
		try {
			JSONObject streamTags = messages.getJSONObject("streamSubDomains");
			JSONArray elements = streamTags.getJSONArray("element");
			for(int i = 0 ; i < elements.length() ; i++){
				String tagCode = elements.getJSONObject(i).getString("codSubDomain");
