						...
						function($scope, $route) {
							...
							$scope.isMenuActive = function(menuItem) {
								...
								case 'dashboard':
									...
									break;
								case 'streams':
									result = ($scope.managementTab == 'streams'
											|| $scope.managementTab == 'editStream'
											|| $scope.managementTab == 'viewStream'
											|| $scope.managementTab == 'newStream' || $scope.managementTab == 'newStreamInternal');
									break;
								case 'virtualentities':
									result = ($scope.managementTab == 'virtualentities'
											|| $scope.managementTab == 'editVirtualentity'
											|| $scope.managementTab == 'viewVirtualentity' || $scope.managementTab == 'newVirtualentity');
									break;
								case 'datasets':
									result = ($scope.managementTab == 'datasets'
											|| $scope.managementTab == 'editDataset'
											|| $scope.managementTab == 'viewDataset'
											|| $scope.managementTab == 'newDataset' || $scope.managementTab == 'uploadDataset');
									break;
								default:
									break;
								}
...
appControllers.controller('ManagementCtrl', [ '$scope', '$route',
		'fabricAPIservice',
		function($scope, $route, fabricAPIservice, filterFilter) {
		} ]);

appControllers
		.controller(
				'ManagementDashboardCtrl',
				[
						'$scope',
						'$route',
						'fabricAPIservice',
...
appControllers
		.controller(
				...
				[
						...
						'fabricAPIservice',
						'info',
						function($scope, $route, $location, fabricAPIservice,
								info, filterFilter) {
							$scope.tenantCode = $route.current.params.tenant_code;
							...
							console.log("isOwner", info
									.isOwner($scope.tenantCode));
							...
							fabricAPIservice
									.getStreams($scope.tenantCode)
									.success(
											function(response) {

												var responseList = Helpers.util
														.initArrayZeroOneElements(response.streams.stream);
												for (var i = 0; i < responseList.length; i++) {
													if (responseList[i].codiceTenant == $scope.tenantCode) {
														if (!responseList[i].deploymentStatusCode
																|| responseList[i].deploymentStatusCode == null)
															responseList[i].deploymentStatusCode = Constants.STREAM_STATUS_DRAFT;
														responseList[i].statusIcon = Helpers.stream
																.statusIcon(responseList[i]);
														$scope.streamsList
																.push(responseList[i]);
													}
												}
												// $scope.streamsList =
												// Helpers.util.initArrayZeroOneElements(response.streams.stream);
												$scope.totalItems = $scope.streamsList.length;
												// $scope.filteredStreamsList =
												// $scope.streamsList.slice(($scope.currentPage
												// - 1) * $scope.pageSize,
												// $scope.currentPage *
												// $scope.pageSize);
											});
							...
							$scope.selectPage = function() {
								// $scope.filteredStreamsList =
								// $scope.streamsList.slice(($scope.currentPage
								// - 1) * $scope.pageSize, $scope.currentPage *
								// $scope.pageSize);
							};
							...
							$scope.searchCodeFilter = function(stream) {
								...
								return !$scope.codeFilter
										|| keyword.test(stream.codiceStream);
							};

							$scope.searchStatusFilter = function(stream) {
								var keyword = new RegExp($scope.statusFilter,
										'i');
								return !$scope.statusFilter
										|| keyword
												.test(stream.deploymentStatusDesc);
							};
							...
							$scope.canEdit = function() {
								if ($scope.selectedStreams.length == 1
										&& $scope.selectedStreams[0].deploymentStatusCode == Constants.STREAM_STATUS_DRAFT) {
									return true;
								}
								return false;
							};
							...
							$scope.editStream = function() {
								if ($scope.selectedStreams.length === 1) {
									...
									$location
											.path('management/editStream/'
													+ $scope.selectedStreams[0].codiceTenant
													+ '/'
													+ $scope.selectedStreams[0].codiceVirtualEntity
													+ '/'
													+ $scope.selectedStreams[0].codiceStream);
								} else {
									// FIXME error message...
								}
							};
							$scope.deleteStream = function() {
								...
								if ($scope.selectedStreams.length > 0) {
									...
									// $location.path('management/editStream/'+$scope.selectedStreams[0].codiceTenant
									// +'/'+$scope.selectedStreams[0].codiceVirtualEntity+'/'+$scope.selectedStreams[0].codiceStream);
								} else {
									// FIXME error message...
								}
							};
		...
		.controller(
				...
				[
						...
						function($scope, $route, $location, fabricAPIservice,
							...
							fabricAPIservice
									.getVirtualentities($scope.tenantCode)
									.success(
											function(response) {
												console
														.log(response.virtualEntities.virtualEntity);
									...
									.success(
											function(response) {
												for (var int = 0; int < response.streamDomains.element.length; int++) {
													$scope.domainList
															.push(response.streamDomains.element[int].codDomain);
												}
											});
							...
							$scope.createStream = function(virtualentity,
									stream) {
								stream.fps = 0;
								stream.saveData = 0;
								...
								stream.accettazionePrivacy = $scope.accettazionePrivacy
										& $scope.accettazioneResponsability;

								var newStream = new Object();
								newStream.stream = stream;

								var promise = fabricAPIservice.createStream(
										$scope.tenantCode,
										virtualentity.codeVirtualEntity,
										newStream);
								promise.then(function(result) {
									console.log("result qui ", result);
									$location.path('management/editStream/'
											+ $scope.tenantCode + '/'
											+ virtualentity.codeVirtualEntity
											+ '/'
											+ newStream.stream.codiceStream);
								}, function(result) {
									$scope.creationError = angular
...
appControllers
		.controller(
				...
				[
						...
						'$timeout',
						"$filter",
						function($scope, $routeParams, fabricAPIservice, info,
								$timeout, $filter) {
							...
							$scope.addStreamToArray = function() {
								...
								$scope.internalStreams
										.push($scope.streamSelectedItem);
								$scope
										.loadStreamComponents($scope.streamSelectedItem);
								$scope.streamSelectedItem = null;
							};
							...
							$scope.valideteSiddhi = function() {
								...
								if ($scope.stream.componenti == null
										|| $scope.stream.componenti.element == null
										|| $scope.stream.componenti.element.length == 0) {
									$scope.validationRes = 1;
									$scope.errorMsg = "STREAM_SIDDHI_INSERT_COMPONENT";
									return;
								}
								if ($scope.streamSiddhiQuery == null
										|| $scope.streamSiddhiQuery
												.indexOf("outputStream") == -1) {
									$scope.validationRes = 1;
								...
								for ( var st in $scope.internalStreams) {
									...
									siddhiStreamDefinitions += "define stream "
											+ "input"
											+ st
											+ " (meta_source string, time string ";
									if ($scope.internalStreams[st].componenti != null
											&& $scope.internalStreams[st].componenti.element != null) {
								...
								siddhiStreamDefinitions += " define stream "
										+ "outputStream(meta_source string, time string ";
								if ($scope.stream.componenti != null
										&& $scope.stream.componenti.element != null) {
							...
							fabricAPIservice
									...
									.success(
											function(response) {
												...
												for (var i = 0; i < responseList.length; i++) {
													if (responseList[i].deploymentStatusCode
															&& responseList[i].deploymentStatusCode == Constants.STREAM_STATUS_INST
															&& (responseList[i].visibility
																	&& responseList[i].visibility == "public" || responseList[i].codiceTenant == $routeParams.tenant_code)) {
														$scope.streamsList
																.push(responseList[i]);
													}
												}
									...
									.success(
											function(response) {
												for (var int = 0; int < response.streamTags.element.length; int++) {
													$scope.tagList
															.push(response.streamTags.element[int].tagCode);
												}
											});
							...
							$scope.loadStream = function() {
								fabricAPIservice
										.getStream($routeParams.tenant_code,
												$routeParams.entity_code,
												$routeParams.stream_code)
										.success(
												function(response) {
													...
													console
															.debug(
																	"$scope.stream internal before clean",
																	$scope.stream);
													if (!$scope.stream.streamInternalChildren
															|| !$scope.stream.streamInternalChildren.streamChildren) {
														$scope.stream.streamInternalChildren = {};
														$scope.stream.streamInternalChildren.streamChildren = [];
													}

													$scope.stream.streamInternalChildren.streamChildren = Helpers.util
															.initArrayZeroOneElements($scope.stream.streamInternalChildren.streamChildren);
													...
													for (var i = 0; i < $scope.stream.streamInternalChildren.streamChildren.length; i++) {
														...
														$scope
																.loadStreamComponents(existingStream);
													}

													if ($scope.stream.internalQuery
															&& $scope.stream.internalQuery["@nil"]) {
														$scope.stream.internalQuery = null;
													}
													console
															.debug(
																	"$scope.stream.internalQuery ",
																	$scope.stream.internalQuery);
													...
													console
															.debug(
																	"$scope.stream internal",
																	$scope.stream);
													if (!$scope.stream.streamTags)
														$scope.stream.streamTags = new Object();
													$scope.stream.streamTags.tag = Helpers.util
															.initArrayZeroOneElements($scope.stream.streamTags.tag);

													if ($scope.stream.componenti == null)
														$scope.stream.componenti = new Object();
													$scope.stream.componenti.element = Helpers.util
															.initArrayZeroOneElements($scope.stream.componenti.element);

													if (!$scope.stream.deploymentStatusCode
															|| $scope.stream.deploymentStatusCode == null)
							...
							$scope.loadStreamComponents = function(
									existingStream) {
								fabricAPIservice
										.getStream(
												existingStream.codiceTenant,
												existingStream.codiceVirtualEntity,
												existingStream.codiceStream)
										.success(
												function(response) {
													...
													for (var i = 0; i < $scope.internalStreams.length; i++) {
														if ($scope.internalStreams[i].idStream == stream.idStream) {
															...
															$scope.internalStreams[i].componenti.element = Helpers.util
																	.initArrayZeroOneElements(stream.componenti.element);
														}
													}
												});
							...
							$scope.addComponent = function() {
								...
								if ($scope.newComponent
									...
									if (!found) {
										...
										$scope.insertComponentErrors
												.push('MANAGEMENT_EDIT_STREAM_ERROR_COMPONENT_NAME_UNIQUE');
									}
								} else
									$scope.insertComponentErrors
											.push('MANAGEMENT_EDIT_STREAM_ERROR_COMPONENT_NAME_REQUIRED');

								console.log("$scope.newComponentDataType",
										$scope.newComponentDataType);
								if ($scope.newComponentDataType == null
										|| $scope.newComponentDataType == "") {
									$scope.insertComponentErrors
											.push('MANAGEMENT_EDIT_STREAM_ERROR_COMPONENT_TYPE_REQUIRED');
								}

								if ($scope.insertComponentErrors.length == 0) {
									$scope.stream.componenti.element
											.push($scope.newComponent);
									$scope.newComponent = null;
								}
								return false;
							};
							...
							$scope.removeComponent = function(index) {
								$scope.validationRes = 2;
								$scope.stream.componenti.element.splice(index,
										1);
								return false;
							};
							...
							$scope.addTag = function() {
								if ($scope.newTag) {
									...
									if (!found)
										$scope.stream.streamTags.tag.push({
											"tagCode" : $scope.newTag
										});
								}
								$scope.newTag = null;
								return false;
							...
							$scope.canUnistall = function() {
								if ($scope.stream
										&& $scope.stream.deploymentStatusCode == Constants.STREAM_STATUS_INST)
									return true;
								return false;
							};
							...
							$scope.updateStream = function() {
								...
								if ($scope.validationRes != 0
										&& $scope.stream.codiceVirtualEntity == "internal") {
									...
									// newStream.stream.internalQuery =
									// $scope.streamSiddhiQuery;
									...
									if (!$scope.stream.componenti.element
										...
										$scope.warningMessages
												.push("MANAGEMENT_EDIT_STREAM_WARNING_NO_COMPONENTS");
									}

									Helpers.util.scrollTo();
									if (!$scope.updateWarning) {

										var promise = fabricAPIservice
							...
							var updateLifecycle = function(action) {
								console.log("updateLifecycle stream",
										$scope.stream);
								console.log("updateLifecycle action", action);
								$scope.updateInfo = null;
								$scope.updateError = null;
								Helpers.util.scrollTo();
								var promise = fabricAPIservice.lifecycleStream(
										action, $scope.stream);
								promise.then(function(result) {
									console.log("result updateLifecycle ",
											result);
									// $scope.updateInfo =
									// angular.fromJson(result.data); //FIXME
									// when the api will be ready
									$scope.updateInfo = {
										status : result.status
									};
									$scope.loadStream();
...
appControllers
		.controller(
				'ManagementVirtualentityListCtrl',
				[
						'$scope',
						'$route',
						'$location',
						...
						function($scope, $route, $location, fabricAPIservice,
							...
							console.log("info.isOwner( $scope.tenantCode);",
									info.isOwner($scope.tenantCode));
							$scope.isOwner = function() {
								return info.isOwner($scope.tenantCode);
							};
							...
							fabricAPIservice
									...
									.success(
											function(response) {
												// Dig into the responde to get
												// the relevant data
												console.log("response",
														response);
												if (response.virtualEntities == null)
													response.virtualEntities = {};
												$scope.virtualentitiesList = Helpers.util
														.initArrayZeroOneElements(response.virtualEntities.virtualEntity);
												$scope.totalItems = $scope.virtualentitiesList.length;
												// $scope.filteredVirtualentitiesList
												// =
												// $scope.virtualentitiesList.slice(($scope.currentPage
												// - 1) * $scope.pageSize,
												// $scope.currentPage *
												// $scope.pageSize);
											});
							...
							$scope.searchCodeFilter = function(virtualentity) {
								...
								return !$scope.codeFilter
										|| keyword
												.test(virtualentity.codeVirtualEntity);
							};
							...
							$scope.searchStatusFilter = function(virtualentity) {
								...
								return !$scope.statusFilter
										|| keyword
												.test(virtualentity.deploymentStatusDesc);
							};

							$scope.$watch('codeFilter', function(newCode) {
								$scope.currentPage = 1;
								// $scope.totalItems =
								// $scope.filteredVirtualentitiesList.length;
							});
							...
							$scope.isSelected = function(virtualentity) {
								return $scope.selectedVirtualentities
										.indexOf(virtualentity) >= 0;
							};
							...
							var updateSelected = function(action, virtualentity) {
								...
								if (action === 'remove'
										&& $scope.selectedVirtualentities
												.indexOf(virtualentity) !== -1) {
									$scope.selectedVirtualentities.splice(
											$scope.selectedVirtualentities
													.indexOf(virtualentity), 1);
								}
							};
							...
							$scope.editVirtualentity = function() {
								if ($scope.selectedVirtualentities.length === 1) {
									$location
											.path('management/editVirtualentity/'
													+ $scope.selectedVirtualentities[0].codiceTenant
													+ '/'
													+ $scope.selectedVirtualentities[0].codeVirtualEntity);
								} else {
									// FIXME error message...
								}
							};
							$scope.deleteStream = function() {
								...
								if ($scope.selectedVirtualentities.length > 0) {
									...
									// $location.path('management/editStream/'+$scope.selectedVirtualentities[0].codiceTenant
									// +'/'+$scope.selectedVirtualentities[0].codiceVirtualEntity+'/'+$scope.selectedVirtualentities[0].codiceStream);
								} else {
									// FIXME error message...
								}
							};
...
appControllers
		.controller(
				'ManagementNewVirtualentityCtrl',
				[
						'$scope',
						'$route',
						'$location',
						...
						function($scope, $route, $location, fabricAPIservice,
							...
							fabricAPIservice
									...
									.success(
											function(response) {
												...
												// $scope.typesList =
												// response.tipoVirtualEntity.element;
											});
							...
							// $scope.validationPatternUUID =
							// Constants.VALIDATION_PATTERN_UUID;
								...
								return {
									test : function(value) {
										if ($scope.selectedType != Constants.VIRTUALENTITY_TYPE_DEVICE_ID)
											...
											return Constants.VALIDATION_PATTERN_UUID
													.test(value);
									}
								};
							})();
							...
							$scope.isCodeRequired = function() {
								console
										.debug(
												"isCodeRequired function $scope.selectedType ::",
												$scope.selectedType);
								if (!$scope.selectedType) {
									return false;
								}
								// return $scope.selectedType ==
								// Constants.VIRTUALENTITY_TYPE_DEVICE_ID ||
								// $scope.selectedType ==
								// Constants.VIRTUALENTITY_TYPE_DEVICE_ID;
								return true;
							};
							...
							$scope.isCategoryRequired = function() {
								console
										.debug(
												"isCategoryRequired function $scope.selectedType ::",
												$scope.selectedType);
								if (!$scope.selectedType) {
									return false;
								}
							...
							$scope.selectTypeChange = function() {
								console
										.debug(
												"selectTypeChange function $scope.selectedType ::",
												$scope.selectedType);
								$scope.codeVirtualEntity = "";
								$scope.selectedCategory = null;
								return true;
							...
							$scope.createVirtualentity = function(virtualentity) {
								...
								console.log("newVirtualentity",
										newVirtualentity);

								var promise = fabricAPIservice
										.createVirtualentity($scope.tenantCode,
												$scope.codeVirtualEntity,
												newVirtualentity);
								promise
										.then(
		...
		.controller(
				...
				[
						...
						function($scope, $routeParams, fabricAPIservice, info) {
								...
								fabricAPIservice
										...
										.success(
												function(response) {
													...
													$scope.virtualentity.virtualEntityPositions.position = Helpers.util
															.initArrayZeroOneElements($scope.virtualentity.virtualEntityPositions.position);
													...
													Helpers.util
															.cleanNilInField($scope.virtualentity);
												});
							};
							...
							$scope.isDevice = function() {
								if (!$scope.virtualentity
										|| !$scope.virtualentity.tipoVirtualEntity)
									return false;
							...
							$scope.updateVirtualentity = function() {
								...
								console.log("newVirtualentity",
										newVirtualentity);
								var promise = fabricAPIservice
										.updateVirtualentity(newVirtualentity);
								promise.then(function(result) {
									console.log("result qui ", result);
									// $scope.updateInfo =
									// angular.fromJson(result.data); //FIXME
...
appControllers.controller('ManagementChooseTenantCtrl', [ '$scope',
		'fabricAPIservice', function($scope, fabricAPIservice) {
...
/* Dataset */
appControllers.controller('ManagementDatasetListCtrl', [
		'$scope',
		'$route',
		'$location',
		'fabricAPImanagement',
		'info',
		function($scope, $route, $location, fabricAPImanagement, info,
				filterFilter) {
			$scope.tenantCode = $route.current.params.tenant_code;
			...
			fabricAPImanagement.getDatasets($scope.tenantCode).success(
					function(response) {
			...
			$scope.selectPage = function() {
				// $scope.filteredStreamsList =
				// $scope.streamsList.slice(($scope.currentPage - 1) *
				// $scope.pageSize, $scope.currentPage * $scope.pageSize);
			};
			...
			$scope.searchNameFilter = function(dataset) {
				...
				return !$scope.nameFilter
						|| (dataset.info.datasetName && keyword
								.test(dataset.info.datasetName));
			};
			...
			var updateSelected = function(action, dataset) {
				if (action === 'add'
						&& $scope.selectedDatasets.indexOf(dataset) === -1) {
					$scope.selectedDatasets.push(dataset);
				}
				if (action === 'remove'
						&& $scope.selectedDatasets.indexOf(dataset) !== -1) {
					$scope.selectedDatasets.splice($scope.selectedDatasets
							.indexOf(dataset), 1);
				}
			};
			...
			$scope.editDataset = function() {
				if ($scope.selectedDatasets.length === 1) {
					...
					$location.path('management/editDataset/'
							+ $scope.tenantCode + '/'
							+ $scope.selectedDatasets[0].datasetCode);
				} else {
					// FIXME error message...
				}
			};
				...
				[
						...
						function($scope, $route, $location, fabricAPIservice,
							...
							$scope.createDataset = function(dataset) {
								...
								var promise = fabricAPImanagement
										.createDataset($scope.tenantCode,
												newDataset);
								promise.then(function(result) {
									console.log("result qui ", result);
									$location.path('management/editDataset/'
...
appControllers
		.controller(
				...
				[
						...
						'$location',
						'info',
						function($scope, $routeParams, fabricAPIservice,
								fabricAPImanagement, $location, info) {
							$scope.tenantCode = $routeParams.tenant_code;
							$scope.datasetCode = $routeParams.entity_code;
							$scope.downloadCsvUrl = Constants.API_MANAGEMENT_DATASET_DOWNLOAD_URL
									+ $scope.tenantCode
									+ '/'
									+ $scope.datasetCode + '/csv';
							...
							// $scope.dataset.columns.column =
							// Helpers.util.initArrayZeroOneElements($scope.dataset.columns.column);

							$scope.loadDataset = function() {
								fabricAPImanagement
										.getDataset($scope.tenantCode,
												$scope.datasetCode)
										.success(
												function(response) {
													console
							...
							$scope.canEdit = function() {
								return ($scope.dataset
										&& $scope.dataset.configData
										&& $scope.dataset.configData.type == "dataset" && $scope.dataset.configData.subtype == "bulkDataset");
							};
							...
							$scope.updateDataset = function() {
								...
								if (!newDataset.info.tags
										&& newDataset.info.tags.length == 0) {
								...
								var promise = fabricAPImanagement
										.updateDataset($scope.tenantCode,
												$scope.datasetCode, newDataset);
							...
							var updateLifecycle = function(action) {
								...
								// Helpers.util.scrollTo();
								// var promise =
								// fabricAPIservice.lifecycleStream(action,
								// $scope.stream);
								// promise.then(function(result) {
								// console.log("result updateLifecycle ",
								// result);
								// //$scope.updateInfo =
								// angular.fromJson(result.data); //FIXME when
								// the api will be ready
...
appControllers
		.controller(
				...
				[
						...
						'readFilePreview',
						'$translate',
						function($scope, $routeParams, fabricAPImanagement,
								info, $upload, readFilePreview, $translate) {
							$scope.tenantCode = $routeParams.tenant_code;
							$scope.datasetCode = $routeParams.entity_code;
							...
							$scope.onFileSelect = function($files) {
								...
								if ($scope.selectedFile != null
										&& $scope.selectedFile.size > Constants.BULK_DATASET_MAX_FILE_SIZE) {
							...
							var readPreview = function() {
								...
								$scope.updateWarning = null;
								readFilePreview
										.readFile($scope.selectedFile, 10000,
												$scope.fileEncoding)
										.then(
												function(contents) {
													...
													$scope.previewLines = Helpers.util
															.CSVtoArray(
																	firstRows
																			.join("\n"),
																	$scope.csvSeparator);
							...
							$scope.uploadData = function() {
								...
								$scope.upload = $upload
										.upload(
												{
													url : Constants.API_MANAGEMENT_DATASET_ADD_DATA_URL
															+ $scope.tenantCode
															+ '/'
															+ $scope.datasetCode
															+ '/',
													...
													method : 'POST',
													data : {
														formatType : $scope.importFileType,
														csvSeparator : $scope.csvSeparator,
														encoding : $scope.fileEncoding,
														skipFirstRow : $scope.csvSkipFirstRow
													},
													file : $scope.selectedFile, // or
																				// list
																				// of
																				// files
																				// ($files)
...
appControllers
		.controller(
				'ManagementNewDatasetWizardCtrl',
				[
						'$scope',
						'$route',
						'$location',
						...
						'$upload',
						function($scope, $route, $location, fabricAPIservice,
								fabricAPImanagement, readFilePreview, info,
								$upload) {
							$scope.tenantCode = $route.current.params.tenant_code;
											...
											function(response) {
												...
												for (var int = 0; int < $scope.dataTypeList; int++) {
													if ($scope.dataTypeList[int].dataType == 'string') {
														console
																.log(
																		"$scope.dataTypeList[int].dataType",
																		$scope.dataTypeList[int].dataType);
								...
								readFilePreview
										...
										.then(
												function(contents) {
													...
													// for (var int = 0; int <
													// firstRows.length; int++)
													// {
													// var items =
													// firstRows[int].split($scope.csvSeparator);
													// console.log("items
													// prima", items);

													// for (var j = 0; j <
													// items.length; j++) {
													// console.log("item prima",
													// items[j]);
													// items[j] =
													// items[j].replace(/^"(.*)"$/,
													// '$1');
													// console.log("item dopo",
													// items[j]);
													// }
													// console.log("items dopo",
													// items);
														...
														for (var int = 0; int < $scope.previewLines[0].length; int++) {
															$scope.previewColumns
																	.push({
																		index : int,
																		sourceColumn : int + 1,
																		fieldName : $scope.previewLines[0][int]
																				.replace(
																						/^"(.*)"$/,
																						'$1'),
																		fieldAlias : $scope.previewLines[0][int]
																				.replace(
																						/^"(.*)"$/,
																						'$1'),
							...
							$scope.refreshColumnOrder = function() {
								...
								if ($scope.previewColumns
										&& $scope.previewColumns.length > 0) {
									var order = 1;
									$scope.metadata.info.fields = [];
									for (var int = 0; int < $scope.previewColumns.length; int++) {
										...
										if (!column.skipColumn) {
											...
											var dataType = column.dataType ? column.dataType.dataType
													: 'string';
											console.log("column.dataType",
													column.dataType);
											console.log("dataType", dataType);
											var measureUnit = column.measureUnit ? column.measureUnit.measureUnit
													: null;
											$scope.metadata.info.fields
													.push({
							...
							$scope.goToRequestor = function() {
								$scope.currentStep = 'requestor';
								refreshWizardToolbar();
							};
							$scope.goToMetadata = function() {
								$scope.currentStep = 'metadata';
								refreshWizardToolbar();
							};
							$scope.goToUpload = function() {
								$scope.currentStep = 'upload';
								refreshWizardToolbar();
							};
							...
							$scope.createDataset = function(dataset) {
								...
								$scope.upload = $upload
										.upload(
												{
													url : Constants.API_MANAGEMENT_DATASET_URL
															+ $scope.tenantCode
															+ '/',
													...
													data : {
														dataset : newDataset,
														formatType : $scope.metadata.info.importFileType,
														csvSeparator : $scope.csvSeparator,
														encoding : $scope.fileEncoding,
														skipFirstRow : $scope.csvSkipFirstRow
													},
										...
										.success(
												function(data, status, headers,
													...
													if (data.errors
														...
														$location
																.path('/management/viewDataset/'
																		+ $scope.tenantCode
																		+ "/"
																		+ data.metadata.datasetCode);
													}
